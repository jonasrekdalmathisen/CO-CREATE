...
#libraries:
pacman::p_load(COINr, countrycode, reactable, tidyverse, Pheatmap, RColorBrew)
```
...
policies <- policies %>% filter(!is.na(BENCHMARK), is.na(`Policy expired (year)`), `ACTION OR EVALUATION` == "Policy Action", PolicyLevel == "National")
...
Presence_100 <- Presence_100 %>% mutate(across(everything(), ~replace_na(., 0)))
...
```{r}
AggHiearchy <- all_policies %>% distinct(SubPArea, BenchmarkID, PolicyArea, PolicyAreaID)

...
```{r Packages}
#libraries:
pacman::p_load(COINr, ggbiplot, countrycode, reactable, tidyverse, RColorBrewer, readxl)
```
...
Benchmark_raw <- read_excel("rescaledN.xlsx", 
...
Alternative model: distance to target

```{r}
dist2targ_complex <- ComplexModel

#Only M&E excluded indicators included

dist2targ_complex$Method$normalise$ntype <- "dist2targ"

#Apply strict model witout M&E to assess change

dist2targ_complex <- regen(dist2targ_complex, quietly=F)

getResults(dist2targ_complex, tab_type = "Aggregates" %>% reactable(compact=T)
...
getResults(z_score, tab_type = "Aggregates", out2 = "df")

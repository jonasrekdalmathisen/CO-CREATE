...
MissingWNAS <- NPI_f$Analysis$Raw$MissDatSummary %>% roundDF(1) 

write.csv(MissingWNAS, "NA MissingAnalysis.csv")
...
NPI_f <- aggregate(NPI_f, dset="Normalised", agtype_bylevel = c("arith_mean", "harm_mean", "harm_mean"), avail_limit = 0.7, )
```


```{r}
#Results
perc <- getResults(NPI_f, tab_type = "Aggregates")
write.csv(perc, file="PercCalcMean_dataavail.csv", row.names=T)
